...
if players == "1":
    ...
    print("player 2 look away just now")
    print("player 1 think of a sequence")
    answer = get_sequence()
    for a in range(100):
        print()
    print("player 2 turn to guess")

attempt = ""
lives = STARTING_LIVES
guesses_so_far = []

while lives > 0 and attempt != answer:
    if lives < 0:
        print("answer was", answer)
    else:
        print("you have", lives, "tries left")
    attempt = get_sequence()
    if attempt == answer:
        print("you got it!")
    lives -= 1
    guesses_so_far.append(attempt)
    past_answers(guesses_so_far, answer)
    '''
...
'''
sequence = []
while len(sequence) < ROW_SIZE:
    prompt = "write a number from 1 to " + str(NUMBER_OPTIONS) + ": "
    answer = input(prompt)
    if not answer.isdigit():
        print("write a number")
    elif int(answer) not in range(1, NUMBER_OPTIONS + 1):
        print("must be between 1 and ", NUMBER_OPTIONS)
    else:
...
def start_game():1
    ...
    while True:
        for event in pg.event.get():
            if event.type == pg.KEYDOWN:
                if event.key == pg.K_1:
                    correct_seq = random_sequence()
                    draw()
                    return correct_seq
                elif event.key == pg.K_2:
                    correct_seq = select_colours()
                    draw()
                    return  correct_seq
                else:
                    draw()
